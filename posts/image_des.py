...
def design(link):
    ...
    place=(int(float(croped.size[0]/2-150)),imageTop+50)
    aps=aps.resize((int(float(aps.size[0]/1.2)),int(float(aps.size[1]/1.2))), Image.ANTIALIAS)
    apple=bi.copy()
    ...
    blurred = blurred.filter(ImageFilter.BoxBlur(20))
  ...
  # draw.text((x, y),"Sample Text",(r,g,b))
    draw.text((imageLeft/2-25, imageTop+10),"حمل التطبيق الان",(0, 0, 0),font=font)
    
    qr = qr.resize((90,90), Image.ANTIALIAS)
    qr_add=apple.copy()
    logo_w=qr.size[0]
    logo_h=qr.size[1]
    imageLeft1 = croped.size[0]-logo_w-52
    imageTop2 = croped.size[1]-logo_h-22
